...
open class TextDecoder: TextDecoding, WhisperMLModel {
    ...
    // MARK: - Speculative Decoding Methods

    /// Generate draft tokens using the draft decoder
    /// - Parameters:
    ///   - encoderOutput: The encoder output embeddings
    ///   - decoderInputs: Current decoder inputs with context
    ///   - maxLength: Maximum number of draft tokens to generate
    ///   - options: Decoding options
    /// - Returns: Array of draft token IDs
    /// - Throws: WhisperError if draft decoder is not available
    public func draftDecode(
        encoderOutput: MLMultiArray,
        decoderInputs: DecodingInputs,
        maxLength: Int,
        options: DecodingOptions
    ) async throws -> [Int] {
        guard let draftDecoder = self.draftDecoder else {
            throw WhisperError.assistantModelNotLoaded()
        }

        var draftTokens: [Int] = []
        var currentInputs = decoderInputs
        ...
        for _ in 0..<maxLength {
            ...
            guard let output = try await draftDecoder.predictLogits(
            ...
            guard let logits = output.logits else { break }
            let nextToken = greedySample(logits: logits)

            // Check for end token
            if let tokenizer = self.tokenizer, nextToken == tokenizer.specialTokens.endToken {
                break
            }
            ...
            currentInputs.inputIds[0] = NSNumber(value: nextToken)
    ...
    /// - Parameters:
    ///   - draftTokens: Tokens generated by draft model
    ///   - encoderOutput: The encoder output embeddings
    ///   - decoderInputs: Current decoder inputs
    ///   - threshold: Probability threshold for acceptance
    /// - Returns: Tuple of (accepted tokens, corrected token if any)
    public func verifyTokens(
        draftTokens: [Int],
        encoderOutput: MLMultiArray,
        decoderInputs: DecodingInputs,
        threshold: Float
    ) async throws -> (accepted: [Int], correction: Int?) {
        ...
        for draftToken in draftTokens {
            ...
            let probability = tokenProbability(logits: logits, token: draftToken)

            if probability >= threshold {
                // Accept draft token
                acceptedTokens.append(draftToken)

                // Update inputs for next verification
                currentInputs.inputIds[0] = NSNumber(value: draftToken)
                let cacheLen = currentInputs.cacheLength[0].intValue
                currentInputs.cacheLength[0] = NSNumber(value: cacheLen + 1)
            } else {
                // Reject - sample correction from main model
                let correctedToken = greedySample(logits: logits)
                return (acceptedTokens, correctedToken)
            }
        }
    ...
    private func greedySample(logits: MLMultiArray) -> Int {
        var maxValue: Float = -Float.infinity
        var maxIndex = 0

        let count = logits.count
        try? logits.withUnsafeBufferPointer(ofType: Float.self) { ptr in
            for i in 0..<count {
                if ptr[i] > maxValue {
                    maxValue = ptr[i]
                    maxIndex = i
    ...
    private func tokenProbability(logits: MLMultiArray, token: Int) -> Float {
        ...
        var sumExp: Float = 0
        var tokenExp: Float = 0

        try? logits.withUnsafeBufferPointer(ofType: Float.self) { ptr in
            for i in 0..<count {
                let exp = expf(ptr[i] - maxLogit)
                sumExp += exp
                if i == token {
                    tokenExp = exp
                }
    ...
    public func decodeText(
        ...
        if options.useSpeculativeDecoding && isSpeculativeDecodingAvailable {
            ...
            do {
                ...
                let logProbs: [[Int: Float]] = allTokens.map { [Int($0): 0.0] }
